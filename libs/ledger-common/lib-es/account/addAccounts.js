 ...
 */
export function groupAddAccounts(existingAccounts, scannedAccounts, context) {
    ...
    existingAccounts.forEach(function (existingAccount) {
        var migrate = findAccountMigration(existingAccount, scannedAccountsWithoutMigrate);
        if (migrate) {
...
export function migrateAccounts(_a) {
    ...
    existingAccounts.forEach(function (existing) {
        var migration = findAccountMigration(existing, scannedAccounts);
...
export function addAccounts(_a) {
    ...
    existingAccounts.forEach(function (existing) {
        var migration = findAccountMigration(existing, selected);
        if (migration) {
    ...
    return uniqWith(newAccounts, sameAccountIdentity).map(function (a) {
        var name = validateNameEdition(a, renamings[a.id]);
        if (name)
